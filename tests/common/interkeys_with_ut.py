...
class InterWithUT:
    def __init__(self, base_url,token):
        """初始化Inter类
        Args:
            base_url: 基础URL，例如 'http://api.example.com'
        """
        self.base_url = base_url
        # jiaqi ota服务 公网地址 "http://36.189.234.237:29305/lumi"
        # jingjie ota接口 服务地址 http://xj7-5.s.filfox.io:29304/lumi/device/applyToken
    ...
    def post(self, url="", path_params=None,params=None,headers = None):
        # 处理路径参数
        if path_params:
        ...
        try:
            if isinstance(params, dict):
                response = self.session.post(full_url, json=params, headers=final_headers)
                logger.info(response.json())
                # print(params)


            else:
                response = self.session.post(full_url, data=params,headers=final_headers)
                logger.info(response.json())
